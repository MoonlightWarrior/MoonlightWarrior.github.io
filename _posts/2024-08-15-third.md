...
우리가 현재 해석하는 것은 $\sum\limits_{i=0}^{s-1}c^{gcd(i,s)}$의 값인 것이다. 그런데 $gcd(i,s)$가 가능한 값은 $s$의 약수 밖에 없다는 것을 생각해서 $d \vert s$인 경우 $gcd(i,s)=d$가 몇 개인지 세워보자. 일단 $gcd(i/d,s/d)$=1이 되는데, $i \leq s$에서 개수가 $\phi(\frac{s}{d})$이 된다. 즉 해당 식을 $\sum\limits_{d|s}\phi(\frac{s}{d})c^{d}$, 순서를 뒤집으면 $\sum\limits_{d \vert s}\phi(d)c^{s/d}$으로 볼 수도 있다. 
...
## [BOJ 15304 (Toys)](https://boj.ma/15304)
