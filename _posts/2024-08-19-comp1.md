...
여기서 $e^{z}$의 역함수인 $\log(z)=|z|+i\arg(z)$으로 정의했을 때의 복소미분가능성은 추후에 다루자(일대일대응 문제를 해결하기 위해서 범위를 principal value로 제한을 해도, $\pi$에서 갑자기 다시 $-\pi$으로 가는 불상사..가 생긴다. 이는 추후에 해결 예정이다. 

<ol start="7">
<li>$f(z)=|z|$는 모든 점에서 complex differnetiable하지 않다. </li>
</ol>

<ol start="8">
<li>$f(z)=\mathcal{Re}(z)$는 모든 점에서 complex differnetiable하지 않다. </li>
</ol>

<ol start="8">
<li>$f(z)=\overline{z}$는 원점 제외 모든 점에서 complex differnetiable하지 않다. </li>
</ol>

<ol start="10">
<li>$f(z)=|z|^{2}$는 원점 제외 모든 점에서 complex differnetiable하지 않다. </li>
</ol>
