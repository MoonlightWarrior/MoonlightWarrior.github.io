...
앞 세 문제와 가장 큰 차이점은 이 경우 모델링해야 하는 결과가 $S_{2n} \times S_{2m}$이라는 점이다. 각각의 순환마디가 정해진 후 각각의 개수를 구하는 건 쉬운데, 그 후 연결하는 점이 고된다..

추후 추가 예정($\Leftrightarrow$ 푸는 중)

## [BOJ 3835 (Alphabet Soup)](https://boj.ma/3835)
